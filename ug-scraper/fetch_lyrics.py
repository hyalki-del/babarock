...
# Precise Path Resolution: Script is in ug-scraper/, playlist.json is in repository root
...
def clean_genius_lyrics(raw_text: str) -> str:
    ...
    # Strip typical Genius header metadata from the first line if present
    if lines and "Lyrics" in lines[0]:
        lines[0] = re.sub(r'.*?Lyrics', '', lines[0])

    text = "\n".join(lines)
    # Remove trailing 'Embed' or numeric embed tags appended by Genius
